...
build_swat_basic <- function(dirname, iyr = 2000, nbyr = 2, wsarea = 45,
                             elev = 500, declat = 45.7, declon = -76,
                             hist_wx = NULL) {
  # Removing the global variables Note. Delete the first line when past
  # supported versions are past v15.1
  swat_general <- NULL
  rm(swat_general)  # A code dummy
  # Correct way to let code know about global variables post 2.15.1
  # if(getRversion() >= '2.15.1') { globalVariables(c('swat_general')) }
  # data(swat_general)
  load(paste(path.package("swatr"), "data/swat_general.rda", sep = "/"))
  dir.create(dirname, showWarnings = FALSE)

  for (file in names(swat_general)) {
    print(file.path(dirname, file))
    cat(unlist(swat_general[file]), file = file.path(dirname, file), sep = "\n")
  }
  ...
  if (length(hist_wx) > 2) {
    ...
    cat(tmp_head, sprintf("%s%005.1f%005.1f\n",
                          ...
                          hist_wx$TMX, hist_wx$TMN),
        file = file.path(dirname,  "tmp.tmp"), sep = "")
    cat(pcp_head, sprintf("%s%005.1f\n", format(hist_wx$DATE, "%Y%j"),
                          hist_wx$PRECIP),
        file = file.path(dirname, "pcp.pcp"), sep = "")
  }

  # Setting up the filecio file
  filecio <- readLines(file.path(dirname, "file.cio"))
  ...
  filecio <- sub("\\d\\d\\d\\d(    \\| IYR)", paste(iyr, "\\1", sep = ""),
                 filecio)
  substr <- sprintf("      %10d    | NBYR", nbyr)
  filecio <- sub("^.*\\| NBYR", substr, filecio, perl = T)
  cat(filecio, file = file.path(dirname, "file.cio"), sep = "\n")

  # Setting up the sub basin files
  num_subbasins <- length(list.files(path = dirname, pattern = "0*.sub"))
  for(subfilename in list.files(path = dirname, pattern = "0*.sub",
